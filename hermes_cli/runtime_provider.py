...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            resolved_provider = auth_mod.resolve_provider(requested_norm)
        except AuthError:
            pass
        else:
            # Let generic custom aliases like "lmstudio" / "ollama" keep searching
            # named custom providers first. If no saved custom provider matches,
            # resolve_runtime_provider() will still fall back to the generic custom
            # path afterwards. But true built-ins like "nous" must not be shadowed
            # by a custom provider with the same raw name.
